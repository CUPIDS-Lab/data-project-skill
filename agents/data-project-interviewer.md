...
5. **Data realities** — single-source or distributed; static or re-released; **sensitivity tier** = public / de-identified / sensitive-human / regulated; whether Indigenous data (CARE) is implicated; whether data is **contributed/crowdsourced**; input licenses; whether the publisher is reachable.
...
Return one markdown block titled **Project Context Profile** with a labeled field per item above, values normalized (sensitivity as the enum; level as L0–L5; now/later/skip per deferred concern), plus an explicit **"Assumptions & defaults applied"** list and a **"Must confirm"** list. Keep it tight; this block is the contract the next phases depend on.
