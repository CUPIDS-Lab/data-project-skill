...
- **Raw data is immutable.** Original sources are committed read-only and never edited in place; every transformation is reproducible from the original. This is why the canonical tree separates `data/raw` from `data/interim` and `data/processed`.
